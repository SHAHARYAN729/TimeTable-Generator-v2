...
Input: The user uploads an .xlsx file containing course names and enrolled student IDs via the React frontend.
Data Extraction: The C++ server receives the file and invokes a Python script (openpyxl) to convert the spreadsheet into a structured JSON file.
Graph Construction: The C++ backend reads the JSON data to build a student-course conflict graph.
Scheduling Algorithm: The engine applies graph coloring to form cliques, checks slot capacity constraints, and uses dynamic programming to optimize slot placement.
Interactive UI: The frontend receives the generated schedule, renders it, and provides manual swap options.
Export: The finalized timetable can be downloaded back into an Excel format directly from the browser.
Getting Started
Prerequisites
Node.js and npm
