...
json calculateClashes() {
    ...
    for(auto &i : studentCourse){
        ...
        for(int k = 0; k < days; k++){
            if(examDayCount[k] == 1) oneExam.push_back(k + 1);
            if(examDayCount[k] == 2) twoExam.push_back(k + 1);
        }
    ...
    json sc2 = json::array();
    for(auto& s : studentClash2) sc2.push_back(s);
    
    json sc3 = json::array();
    for(auto& s : studentClash3) sc3.push_back(s);
    
    json sc4 = json::array();
    for(auto& s : studentClash4) sc4.push_back(s);
...
json generateTimetable(string excel_file, int num_slots, int capacity) {
    ...
    string command = "python convertExcelToJSON.py " + excel_file + " " + json_file;
        ...
        for(auto &i : tempCliques){
            ...
            if(total > capacity){
                ...
                for(int k = 0; k < 250; k++){
                    ...
                    for(auto &j : i){
                        sum += courseCount[j];
                        if(sum <= capacity && total - sum <= capacity) F = 0;
                        if(F == 0) break;
                    }
                    if(F == 0) break;
                }
    ...
    for(auto &c : cliques){
        ...
        for(int k = 0; k < 100; k++){
            ...
            for(auto &studentPair : studentCourse){
                ...
                int sze = v.size();
                for(int j = 1; j < sze; j++){
                    if(v[j] % 2 == 1 && v[j] == v[j - 1] + 1) clash2++;
                }
                for(int j = 3; j < sze; j++){
                    if(v[j] % 2 == 1 && v[j] == v[j - 3] + 3) clash4++;
                }
            }
...
json swapTimetable(int swap_type, const json& params) {
    ...
    if (swap_type == 1) {
        ...
        for(int i = 0; i < slots; i++) {
            ...
            if(F && i != curSlot) positions.push_back(i);
        }
...
json getAvailablePositions(const string& courseName) {
    ...
    for(int i = 0; i < slots; i++) {
        ...
        if(F && i != curSlot) positions.push_back(i + 1);  // Adding 1 to convert to 1-indexed slots
    }
